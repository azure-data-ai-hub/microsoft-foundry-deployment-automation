...
| Permissions | **Contributor** + **User Access Administrator** (or **Owner**) on the target subscription/resource group for the identity that deploys (needed for `deployRoleAssignments = true`). |
...
|---|---|---|
| `validate` | PR to `main`, manual dispatch | `az deployment sub validate` across DEV/STG/PROD matrix |
| `whatif` | PR to `main` | What-If analysis, posted as PR comment + uploaded artifact |
| `deploy-dev` → `deploy-stg` → `deploy-prod` | Push to `main` | Sequential deploy with GitHub Environment approval gates |
| `deploy-manual` | `workflow_dispatch` | On-demand deploy to a chosen environment (`DEV`/`STG`/`PROD`/`PROD-SECONDARY-REGION`), with optional `region` input to override the target Azure region at deploy time |
